...
update_neg_bin_ll_onestep(PyObject *self, PyObject *args) {
  ...
  double* slice_cases = malloc(sizeof(*slice_cases) * window_size);
  int i = 0;
  ...
  double* tau_window = malloc(sizeof(*tau_window) * window_size);
  i = 0;
  ...
  double* sum_tau_window = malloc(sizeof(*sum_tau_window) * window_size);
  i = 0;
  ...
  double* log_tau_window = malloc(sizeof(*log_tau_window) * window_size);
  i = 0;
  ...
  double* ll_normalizing = malloc(sizeof(*ll_normalizing) * window_size);
  i = 0;
  ...
  double* loggamma_slice_cases_phi = malloc(sizeof(*loggamma_slice_cases_phi) * window_size);
  for (int index=0; index<window_size; index++) {
  ...
  double* log_phi_r_tau_window = malloc(sizeof(*log_phi_r_tau_window) * window_size);
  for (int index=0; index<window_size; index++) {
...
update_neg_bin_deriv_onestep(PyObject *self, PyObject *args) {
  ...
  double* inv_phi_r_tau_window = malloc(sizeof(*inv_phi_r_tau_window) * window_size);
  double* inv_phi_r_tau_window2 = malloc(sizeof(*inv_phi_r_tau_window2) * window_size);
  double* log_phi_r_tau_window = malloc(sizeof(*log_phi_r_tau_window) * window_size);
